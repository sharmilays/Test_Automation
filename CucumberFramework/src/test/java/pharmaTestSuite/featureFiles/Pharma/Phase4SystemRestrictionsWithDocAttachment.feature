@Phase4SystemRestrictionsWithDocAttachmentfeature
Feature: 373259 System Restrictions for Document attachment for refills/duplicate Initial Orders

  @489147
  Scenario Outline: SRWithDocAttachment489147TC_Agent attaches an original enrollment form or RX document already used previously to process custom order on shipped order To Verify Create Orderbutton is not available Supervisor Override button is available
    ...
    Examples: 
      | TC_ID                      |  | TestDataSheet       |
      | SRWithDocAttachment_489147 |  | SRWithDocAttachment |

  @489145
  Scenario Outline: SRWithDocAttachment489145TC_Agent attaches an original enrollment form or RX document already used previously to process custom order on shipped order To Verify system does not allow to create the order with appropriate incomplete OVM message
    Given TestCaseID Is "<TC_ID>" and TestDataSheet Is "<TestDataSheet>"
    And open Browser and enter url
    When Enter username and password
    And Click On Login Button
    ...
    Examples: 
      | TC_ID                      |  | TestDataSheet       |
      | SRWithDocAttachment_489145 |  | SRWithDocAttachment |
