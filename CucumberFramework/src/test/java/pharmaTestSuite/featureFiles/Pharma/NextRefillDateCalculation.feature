@NextRefillDateCalculationfeature
Feature: Next Refill Date Calculation
  ...
  Scenario Outline: Verify that APPROX. NEXT REFILL DATE field is blank in orders list view Refill remaining is equal to 0 for the recent order
    ...
    Examples: 
      | TC_ID             |  | TestDataSheet |
      | NextRefill_191888 |  | NextRefill    |

  @191905
  Scenario Outline: Verify that next refill date is displayed under ‘APPROX. NEXT REFILL DATE’ column for the eligible refill order
    Given TestCaseID Is "<TC_ID>" and TestDataSheet Is "<TestDataSheet>"
    And open Browser and enter url
    When Enter username and password
    And Click On Login Button
